...
Last updated: 2026-08-21 (R14.3 Configurable ICP Administration code-complete, tested, not deployed).
...
- **R14.2 (Configurable ICP Foundation) and R14.3 (Configurable ICP Administration) are both implemented, tested, and committed — NOT deployed to production.** R14.2: table `icp_profiles` + nullable `events.icp_profile_id`, resolver at `src/lib/icp/icp-resolver.ts`, and a fix for a real duplicate-logic bug (`CompanyIntelTab.tsx` and `lead-scoring.ts` had two independently-drifted hardcoded industry keyword lists — now one shared function). R14.3: full ICP Admin UI (`/settings/icp`, `tenant_admin`-only), `/api/icp-profiles/*` CRUD/lifecycle routes, an explicit tenant Default ICP (`tenants.default_icp_profile_id`, migration `0017`) replacing R14.2's "exactly one active = default" rule, event-level ICP assignment, and a qualitative-only (no numeric score, no audit log) ICP Test Mode built into the existing `src/lib/icp/fit.ts` rather than a separate scoring engine. Full detail: [docs/RELEASE-14.3-ICP-ADMIN.md](docs/RELEASE-14.3-ICP-ADMIN.md), [docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md). Migrations `0016` and `0017` have **not** been applied to production RDS. **Per the R14.3 amendment's explicit stop-gate: do not deploy to production or begin R14.4 (agent integration) without further approval.**
...
The S3/Transcribe hotfix is done — deployed and verified live. Two things need your decision next: (1) whether to remove the static AWS access key now sitting in production's `.env.production`, now that the instance-role fallback is confirmed working; (2) R14.2 + R14.3 — both code-complete, tested, and committed, but migrations `0016`/`0017` have not been applied to production RDS and neither has been deployed. R14.4 (wiring a resolved `ICPContext` into Conversation Intelligence) is explicitly gated behind further approval per the R14.3 stop gate.
