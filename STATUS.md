...
Last updated: 2026-08-22 (R14.3.1 + R14.4 — event status fix, Plan & Usage, Platform Admin tenant detail, unified multi-ICP targeting for Events + Campaigns — deployed to production and functionally verified live).

## Current state — read this first

- **R14.3.1 + R14.4 are deployed to production and functionally verified live.** R14.3.1: the event-status display bug is fixed (new `deriveEventStatus()` utility, additive, zero migration) — **confirmed live on real data**: a real production event that had been stuck showing "Upcoming" now correctly shows "Completed." Platform Admin gained a tenant detail page and a real per-tenant event count, Tenant Settings was reorganized, and "Subscription & Usage" was replaced with a real "Plan & Usage" section showing live tenant-scoped numbers (`src/lib/plans.ts`, one new `tenants.plan_name` column). R14.4: Events and Campaigns can both now target multiple ICPs (OR semantics) via new `event_icp_profiles`/`campaign_icp_profiles` join tables and a new `campaigns` entity; a unified resolver `resolveTargetingContext()` replaces the single-ICP logic, with precedence **Event ICPs > Campaign ICPs > Tenant Default ICP > null**. Full Campaign Admin UI at `/settings/campaigns`, plus Campaign Test Mode reusing the existing qualitative matcher. Migrations `0018`–`0020` applied cleanly to production RDS (additive only, verified zero existing rows affected — 9 tenants/7 events/3 ICP profiles/16 users all unchanged; `0019`'s backfill inserted exactly the 2 expected rows). Full production Campaign lifecycle (create → activate → assign ICP → assign to event → verify count) and cross-tenant rejection (403) all confirmed working live. Full detail: [docs/RELEASE-14.3.1-HARDENING.md](docs/RELEASE-14.3.1-HARDENING.md), [docs/RELEASE-14.4-UNIFIED-TARGETING.md](docs/RELEASE-14.4-UNIFIED-TARGETING.md), [docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md). **Per the brief's explicit stop-gate: do not begin agent integration (Conversation Intelligence, Lead Scoring, Follow-Up, CRM Sync, Opportunity, ROI, Orchestrator reading `resolveTargetingContext()`) without further approval.**
- **Production is now running `tradeshow-agent:ff3dce3`** (2026-08-22, on top of `8287357`): adds the Edit Event UI + bidirectional Campaign↔Event assignment — neither R14.3.1 nor R14.4 had specified an event *edit* form, only *create*, discovered when trying to use the new targeting features on existing events. New `GET /api/events/:id`; no schema change. Verified live: editing an existing production event's location and Campaign assignment both saved and persisted correctly. Previous container preserved as `tradeshow-agent-prev-8287357` for instant rollback.
