...
Last updated: 2026-08-22 (R14.3.1 + R14.4 — event status fix, Plan & Usage, Platform Admin tenant detail, unified multi-ICP targeting for Events + Campaigns — implemented, tested, committed; NOT deployed).

## Current state — read this first

- **R14.3.1 + R14.4 are code-complete, tested against an isolated database, and committed — NOT deployed to production, and not yet pushed to origin.** R14.3.1: the event-status display bug is fixed (new `deriveEventStatus()` utility, additive, zero migration), Platform Admin gained a tenant detail page and a real per-tenant event count, Tenant Settings was reorganized, and "Subscription & Usage" was replaced with a real "Plan & Usage" section (`src/lib/plans.ts`, one new `tenants.plan_name` column). R14.4: Events and Campaigns can both now target multiple ICPs (OR semantics) via new `event_icp_profiles`/`campaign_icp_profiles` join tables and a new `campaigns` entity; a unified resolver `resolveTargetingContext()` replaces the single-ICP logic, with precedence **Event ICPs > Campaign ICPs > Tenant Default ICP > null**. Full Campaign Admin UI at `/settings/campaigns`, plus Campaign Test Mode reusing the existing qualitative matcher. Migrations `0018`–`0020` are additive only, verified via a 24-check test run against an isolated database (including a specific test of the `0019` backfill against pre-migration-shape legacy data) plus live browser verification. Full detail: [docs/RELEASE-14.3.1-HARDENING.md](docs/RELEASE-14.3.1-HARDENING.md), [docs/RELEASE-14.4-UNIFIED-TARGETING.md](docs/RELEASE-14.4-UNIFIED-TARGETING.md), [docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md). **Per the brief's explicit stop-gate: do not deploy to production and do not begin agent integration without further approval.**
