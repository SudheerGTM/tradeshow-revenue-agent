...
| **Current production commit** | `8287357` — R14.3.1 (event status fix, Plan & Usage, Platform Admin tenant detail) + R14.4 (unified multi-ICP targeting for Events + Campaigns). No agent code touched. Deployed 2026-08-22. Built on top of `5f230b9` (Apollo status route); previous container preserved as `tradeshow-agent-prev-5f230b9` on the EC2 host for instant rollback. Confirmed via live SSH and functional verification against a real production tenant — including the event-status fix working on a real, previously-affected event. |
...
| **Whether production matches `main`/this branch** | **Yes**, as of `8287357`. |
...
| R14.3.1 | **Product hardening** — event-status fix, Platform Admin tenant detail, Tenant Settings reorg, real Plan & Usage | **Complete, deployed to production (`8287357`), functionally verified live** — see [§19](#19-release-14--configurable-icp) and [docs/RELEASE-14.3.1-HARDENING.md](docs/RELEASE-14.3.1-HARDENING.md) |
| **R14.4** | **Unified Multi-ICP Targeting** — Event + Campaign multi-ICP, `resolveTargetingContext()`, Campaign Admin UI + Test Mode | **Complete, deployed to production (`8287357`), functionally verified live** — see [§19](#19-release-14--configurable-icp) and [docs/RELEASE-14.4-UNIFIED-TARGETING.md](docs/RELEASE-14.4-UNIFIED-TARGETING.md). Stop-gated before agent integration. |
...
**Status: R14.1 through R14.4 are all deployed to production and functionally verified live. Stop-gated before agent integration.**
...
- **R14.3.1** — product hardening, implemented and **deployed to production 2026-08-22** (commit `8287357`): **[docs/RELEASE-14.3.1-HARDENING.md](docs/RELEASE-14.3.1-HARDENING.md)**. Event-status display bug fixed (`deriveEventStatus()`, additive, zero migration — the stored `events.status` column is untouched) — **confirmed live on real data**: a real production event stuck showing "Upcoming" now correctly shows "Completed." Platform Admin gained a tenant detail page (`/admin/tenants/[tenantId]`) and a real per-tenant event count; Tenant Settings reorganized into Overview/Targeting/Platform Health/Administration; "Subscription & Usage" replaced with real "Plan & Usage" numbers (`src/lib/plans.ts` + `tenants.plan_name`, migration `0018`, no billing engine) — verified live against real tenant-scoped data.
- **R14.4** — unified multi-ICP targeting, implemented and **deployed to production 2026-08-22** (commit `8287357`, same deployment window as R14.3.1): **[docs/RELEASE-14.4-UNIFIED-TARGETING.md](docs/RELEASE-14.4-UNIFIED-TARGETING.md)**. Supersedes an earlier Campaign-only design (kept for record in [docs/RELEASE-14.3.1-14.4-ASSESSMENT.md](docs/RELEASE-14.3.1-14.4-ASSESSMENT.md), marked superseded). Events *and* Campaigns can now both target multiple ICPs (OR semantics) via new `event_icp_profiles`/`campaign_icp_profiles` join tables and a new `campaigns` entity (migrations `0019`/`0020`, both additive, `0019`'s backfill specifically tested against pre-migration-shape legacy data — and confirmed correct in production, where it backfilled exactly the 2 pre-existing single-ICP event assignments). New unified resolver `resolveTargetingContext(tenantId, eventId)` — precedence **Event ICPs > Campaign ICPs > Tenant Default ICP > null**. Full `/api/campaigns/*` CRUD/lifecycle + `/settings/campaigns` Admin UI + Campaign Test Mode. Re-verified functionally live in production after deployment: full Campaign lifecycle (create → activate → assign ICP → assign to event → event count confirmed incrementing) and cross-tenant rejection (403) all confirmed working, alongside real concurrent production lead-capture traffic completing successfully with zero errors.
- **Both migrations `0018`–`0020` applied cleanly to production RDS** — additive only, verified zero existing rows affected (9 tenants/7 events/3 ICP profiles/16 users all unchanged before/after).
- **The earlier, less-detailed `docs/RELEASE-14-ICP-PLAN.md` is superseded** — it's now a redirect stub, not a source of truth.
- **STOP gate (per the R14.4 brief's final stop-gate):** do not integrate `resolveTargetingContext()` into any agent (Conversation Intelligence, Lead Scoring, Follow-Up, CRM Sync, Opportunity, ROI, Orchestrator) without further explicit approval.
