...
| **Current production commit** | `864f848` — the `main`/`claude/priceless-keller-10439f` reconciliation merge (`0972810`) plus the documentation-reconciliation commit. Confirmed via live SSH and functional verification. Contains the S3/Transcribe instance-role fix; contains zero Release 14 ICP code (R14.1/14.2/14.3 are all committed on top of this but not deployed). |
...
| **Whether production matches `main`/this branch** | **No, by design.** Production is at `864f848`; `main` has R14.2 + R14.3 committed on top of that. Production intentionally does not have them yet — deployment is explicitly stop-gated per the R14.3 amendment until separately approved. |
...
| R14.2 | **Configurable ICP Foundation** — schema, resolver, duplicate-fit-logic fix | Complete, tested, committed; NOT deployed — see [§19](#19-release-14--configurable-icp) and [docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md) |
| **R14.3** | **Configurable ICP Administration** — Admin UI, `/api/icp-profiles/*` CRUD, explicit tenant Default ICP, event assignment, qualitative Test Mode | **Complete, tested, committed; NOT deployed** — see [§19](#19-release-14--configurable-icp), [docs/RELEASE-14.3-ICP-ADMIN.md](docs/RELEASE-14.3-ICP-ADMIN.md), and [docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md). Stop-gated before production deployment and before R14.4. |
...
**Status: R14.1 (assessment), R14.2 (foundation), and R14.3 (administration) are all complete, tested, and committed. Stop-gated before production deployment and before R14.4.**
...
- **R14.2** — foundation, implemented: **[docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md)**. New `icp_profiles` table + nullable `events.icp_profile_id`, a resolver (`src/lib/icp/icp-resolver.ts`), and the `CompanyIntelTab.tsx`/`lead-scoring.ts` duplicate-logic fix (both now share `src/lib/icp/fit.ts`). Verified against an isolated test database (17/17 checks passed) and a live browser check.
- **R14.3** — administration layer, implemented: **[docs/RELEASE-14.3-ICP-ADMIN.md](docs/RELEASE-14.3-ICP-ADMIN.md)** (see its "Implementation report" section for full detail), **[docs/ICP-ARCHITECTURE.md](docs/ICP-ARCHITECTURE.md)**. Full ICP Admin UI (`/settings/icp`, `tenant_admin`-only), `/api/icp-profiles/*` CRUD/lifecycle routes, an explicit tenant Default ICP (`tenants.default_icp_profile_id`, migration `0017`) replacing R14.2's "exactly one active = default" rule, server-side-validated event-level ICP assignment, and a qualitative-only ICP Test Mode built into the existing `src/lib/icp/fit.ts`. Verified via a 22-check isolated-database script plus a full live browser click-through (create → edit → save → activate → set-default → Test Mode), with tenant isolation (4/4) and Test Mode zero-side-effect behavior (zero audit rows, zero business records) confirmed directly against the database.
- **Neither R14.2 nor R14.3 is deployed to production**, and neither migration (`0016`, `0017`) has been applied to production RDS — see [§22](#22-git--worktree-state).
- **The earlier, less-detailed `docs/RELEASE-14-ICP-PLAN.md` is superseded** — it's now a redirect stub, not a source of truth.
- **STOP gate (per the R14.3 amendment):** do not deploy R14.2/R14.3 to production, apply either migration to production, or begin R14.4 (Conversation Intelligence / Lead Scoring / Follow-Up actually reading a resolved `ICPContext`) without further explicit approval.
