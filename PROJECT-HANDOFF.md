...
| **Current production commit** | `fec78eb` — R14.2 + R14.3 (Configurable ICP Foundation + Administration), deployed 2026-08-21. Previous commit `864f848` preserved as `tradeshow-agent-prev-864f848` on the EC2 host for instant rollback. Confirmed via live SSH and functional verification against a real production tenant. |
...
| **Whether production matches `main`/this branch** | **Yes**, as of `fec78eb`. R14.2 + R14.3 are deployed. |
...
**Status: R14.1 (assessment), R14.2 (foundation), and R14.3 (administration) are all complete, tested, committed, and deployed to production. Stop-gated before R14.4.**
...
- **Deployed to production 2026-08-21** (commit `fec78eb`), per the explicit "Production Deployment Approval — R14.2 + R14.3" document. Migrations `0016`/`0017` applied cleanly to production RDS (additive only, verified zero existing rows affected). Re-verified functionally live against a real production tenant (Demo Logistics) — full lifecycle (create → activate → set-default → event assignment → Test Mode → deactivate-clears-default) and cross-tenant rejection (403) all confirmed working in production. Previous commit `864f848` preserved as `tradeshow-agent-prev-864f848` for instant rollback. One pre-existing, unrelated bug was found during this verification — see [docs/CURRENT-KNOWN-ISSUES.md](docs/CURRENT-KNOWN-ISSUES.md) #6. Full report in [docs/RELEASE-14.3-ICP-ADMIN.md § Production deployment report](docs/RELEASE-14.3-ICP-ADMIN.md).
- **The earlier, less-detailed `docs/RELEASE-14-ICP-PLAN.md` is superseded** — it's now a redirect stub, not a source of truth.
- **STOP gate (per the deployment approval):** do not begin R14.4 (Conversation Intelligence / Lead Scoring / Follow-Up actually reading a resolved `ICPContext`) without further explicit approval.
