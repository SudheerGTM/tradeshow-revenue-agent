...
| **Current production commit** | `864f848` — the `main`/`claude/priceless-keller-10439f` reconciliation merge (`0972810`) plus the documentation-reconciliation commit. Confirmed via live SSH and functional verification. Contains the S3/Transcribe instance-role fix; contains zero Release 14 ICP code. |
...
| **Whether production matches `main`/this branch** | **Yes**, as of `864f848`. R14.2 code/docs are complete but not yet committed — production does not have them, correctly, since they haven't shipped as code yet. |
