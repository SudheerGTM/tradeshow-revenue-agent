...
| **Current production commit** | `ff3dce3` — Edit Event UI + bidirectional Campaign↔Event assignment (new `GET /api/events/:id`, no schema change), on top of `8287357` (R14.3.1 + R14.4). Deployed 2026-08-22. Previous container preserved as `tradeshow-agent-prev-8287357` on the EC2 host for instant rollback. Confirmed via live SSH and functional verification against a real production tenant — editing an existing event's location and Campaign assignment both saved correctly. |
...
| **Whether production matches `main`/this branch** | **Yes**, as of `ff3dce3`. |
