...
**Production status (confirmed via live SSH + functional verification, 2026-08-21):** running `fec78eb` — R14.2 + R14.3 (Configurable ICP Foundation + Administration), deployed and functionally verified live against a real production tenant. Migrations `drizzle/0016_icp_profiles.sql` and `drizzle/0017_icp_default_profile.sql` applied cleanly to production RDS (additive only, verified zero existing rows affected). Previous production commit `864f848` preserved as `tradeshow-agent-prev-864f848` for instant rollback. See `PROJECT-HANDOFF.md` §2 and §22, and `docs/CURRENT-KNOWN-ISSUES.md` #1 for an important correction found during the earlier S3/Transcribe hotfix, and #6 for a pre-existing (unrelated) bug found during this deployment's verification.
