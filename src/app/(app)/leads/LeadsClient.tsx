  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[#0F172A]">Lead Intelligence</h1>
          <p className="text-sm text-[#475569] mt-0.5">All contacts captured at your events</p>
        </div>
        <Link href="/leads/new" className="sm:shrink-0">
          <Button className="w-full sm:w-auto"><Plus className="w-4 h-4" /> Capture Lead</Button>
        </Link>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3">
        <div className="relative flex-1 sm:min-w-48">
          ...
          <input
            ...
            className="w-full bg-white border border-[#E2E8F0] rounded-xl pl-9 pr-3 py-2.5 text-sm text-[#0F172A] placeholder-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-[#00B8D9] focus:border-[#00B8D9] transition"
          />
        </div>
        <div className="grid grid-cols-2 sm:flex gap-3">
          <select
            ...
            className="bg-white border border-[#E2E8F0] rounded-xl px-3 py-2.5 text-sm text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#00B8D9] transition"
          >
          ...
          <select
            ...
            className="col-span-2 sm:col-span-1 bg-white border border-[#E2E8F0] rounded-xl px-3 py-2.5 text-sm text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#00B8D9] transition"
          >
          ...
          <>
            ...
            <div className="md:hidden divide-y divide-[#F8FAFC]">
              {leads.map((lead) => {
                ...
                return (
                  <Link key={lead.id} href={`/leads/${lead.id}`} className="block p-4 active:bg-[#F8FAFC]">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="font-medium text-[#0F172A] truncate">{lead.firstName} {lead.lastName ?? ""}</p>
                        <p className="text-xs text-[#475569] truncate mt-0.5">{lead.companyName}{lead.jobTitle ? ` · ${lead.jobTitle}` : ""}</p>
                      </div>
                      <Badge variant={LEAD_STATUS_COLORS[lead.status] ?? "gray"} className="shrink-0">{lead.status}</Badge>
                    </div>
                    <div className="flex items-center justify-between mt-3">
                      <div>
                        {sc && lead.latestScore != null ? (
                          <div className="flex items-center gap-1.5">
                            <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-semibold" style={{ background: sc.bg, color: sc.text }}>
                              <Star className="w-3 h-3" /> {Math.round(lead.latestScore)}
                            </div>
                            <span className="text-[11px]" style={{ color: sc.text }}>{sc.label}</span>
                          </div>
                        ) : (
                          <span className="text-xs text-[#CBD5E1]">No score yet</span>
                        )}
                      </div>
            ...
            <table className="hidden md:table w-full text-sm">
              <thead>
                <tr className="border-b border-[#F1F5F9] text-left text-xs text-[#94A3B8] uppercase tracking-wider">
                  ...
                  <th className="px-5 py-3 font-medium hidden lg:table-cell">Captured</th>
                </tr>
              </thead>
